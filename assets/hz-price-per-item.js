/**
 * horizon-blocks Phase 13: adapted port of Horizon 68760d9 assets/price-per-item.js.
 *
 * DOUBLE rename (Nathan ruling 2026-07-06): price-per-item collides with Dawn
 * on BOTH the asset filename (Dawn's assets/price-per-item.js is live PDP JS)
 * and the custom-element tag (Dawn's file defines `price-per-item`). This
 * port therefore ships as hz-price-per-item.js defining `hz-price-per-item`.
 *
 * Dawn deviation (the ONLY content change — otherwise byte-verbatim,
 * diff-proven vs stock): the define site (source :139-140) registers
 * `hz-price-per-item` instead of `price-per-item`. CSS class names inside the
 * component stay stock. Lockstep consumers of the renamed tag:
 * snippets/quantity-selector.liquid (markup emits <hz-price-per-item>) and
 * hz-product-form.js (two querySelector retargets, source :834/:870 there).
 *
 * Consumer (per-render module script tag): snippets/quantity-selector.liquid
 * via blocks/quantity.liquid + blocks/buy-buttons.liquid (Phase 13).
 */
...
if (!customElements.get('hz-price-per-item')) {
  customElements.define('hz-price-per-item', PricePerItemComponent);
}
