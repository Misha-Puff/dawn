/**
 * horizon-blocks Phase 13: adapted port of Horizon 68760d9 assets/quick-add.js.
 *
 * UNWIRED from M&P card surfaces (Phase 13 round 3 — Dawn-native quick-add
 * ruling, Nathan 2026-07-07): no card-chain file loads this module anymore —
 * quick-add on cards is Dawn's machinery (assets/quick-add.js +
 * snippets/hz-card-quick-add). Stays in-repo as ported vocabulary
 * (port-everything frame); variant-picker.js's SECTION_ID_MAP
 * 'quick-add-dialog' entry stays too (inert, supports this unwired
 * subsystem). Re-wire = swap the gallery render back to snippets/quick-add.
 *
 * Renamed at the FILE level only: Dawn's assets/quick-add.js owns the stock
 * filename (defines Dawn's `quick-add-modal` element); overwriting it would
 * break live Dawn JS. The element tags defined here are collision-free in
 * Dawn and stay stock: `quick-add-component` (source :280) and
 * `quick-add-dialog` (source :364).
 *
 * Dawn deviation (the ONLY content change — otherwise byte-verbatim,
 * diff-proven vs stock): fetchProductPage (source :180-190) appends
 * `section_id=hz-quick-add-product` to the URL it fetches. Stock fetches the
 * full product page and extracts [data-product-grid-content] from the PDP's
 * product-information-content snippet — but our PDPs are Dawn-native and have
 * no such element. The Section Rendering API fragment of
 * sections/hz-quick-add-product supplies the same contract
 * (product-component[data-product-grid-content] + .product-details children).
 * `?variant=` handling is stock (the param passes through the SRA request).
 *
 * The deviation deliberately sits at the fetch site rather than in the
 * productPageUrl getter (the plan's original sketch): the getter also feeds
 * the user-facing mobile product-header title link
 * (updateQuickAddModal, source :229) — which must stay a clean product URL —
 * and product-card.js's hover/focus prefetch (product-card.js:135) funnels
 * through fetchProductPage too, so patching here keeps that prefetch warming
 * the URL the click path actually uses.
 *
 * In-modal variant changes are the OTHER half of the SRA contract: they go
 * through variant-picker.js's SECTION_ID_MAP ('quick-add-dialog' →
 * 'hz-quick-add-product', Phase-13 one-line addition) — not through this file.
 *
 * Consumers (per-render module script tags): snippets/quick-add.liquid +
 * snippets/quick-add-modal.liquid (Phase 13).
 */
...
export class QuickAddComponent extends Component {
  ...
  async fetchProductPage(productPageUrl) {
    ...
    // Dawn deviation (the ONLY one — see header): fetch the Section Rendering
    // API fragment of sections/hz-quick-add-product instead of the full page.
    const fetchUrl = new URL(productPageUrl, window.location.origin);
    fetchUrl.searchParams.set('section_id', 'hz-quick-add-product');
