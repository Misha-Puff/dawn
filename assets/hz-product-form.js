/**
 * horizon-blocks Phase 13: adapted port of Horizon 68760d9 assets/product-form.js.
 *
 * Renamed at the FILE level only: Dawn's assets/product-form.js owns the stock
 * filename (defines Dawn's `product-form` element, live on every PDP);
 * overwriting it would break live Dawn JS. The element tags defined here are
 * collision-free in Dawn and stay stock: `add-to-cart-component` (source :166)
 * and `product-form-component` (source :912).
 *
 * Dawn deviations (the ONLY content changes — otherwise byte-verbatim,
 * diff-proven vs stock):
 * 1. source :834 — `html.querySelector('price-per-item')` → `'hz-price-per-item'`
 * 2. source :870 — morphTargets entry `['price-per-item', …]` → `['hz-price-per-item', …]`
 * Both retarget the variant-change morph at the renamed element:
 * price-per-item collides with Dawn on BOTH filename and element tag (Dawn's
 * assets/price-per-item.js defines `price-per-item`), so the port ships
 * hz-price-per-item.js defining `hz-price-per-item` (Nathan ruling 2026-07-06:
 * double rename, all consumers in lockstep — hz-price-per-item.js define site,
 * snippets/quantity-selector.liquid markup, and these two selectors). The
 * `refs.pricePerItem` reads are `ref=`-attribute based and unaffected.
 *
 * Inert-by-design paths in Dawn (stock code, no changes; for the record):
 * - #animateFlyToCart (source :104) bails at `!cartIcon`:
 *   `.header-actions__cart-icon` doesn't exist in Dawn's header.
 * - #refreshCart (source :228) always takes the `/cart.json` fetch fallback
 *   (source :237): no `cart-items-component` on any Dawn page.
 * - The `sections` request param (source :407/:594) stays empty for the same
 *   reason — the /cart/add.js POST carries no sections list. Rebuy Smart Cart
 *   intercepts the add at the network layer (window.fetch monkey-patch on
 *   /cart/add pathnames), so drawer behavior is unaffected.
 *
 * Consumers (per-render module script tags, slideshow.liquid:91 precedent):
 * blocks/buy-buttons.liquid + snippets/quick-add.liquid (Phase 13).
 */
...
class ProductFormComponent extends Component {
  ...
  #onProductSelect = async (event) => {
    ...
    try {
      ...
      const newPricePerItem = html.querySelector('hz-price-per-item');
      ...
      if ((isQuantityRulesChanging || isPricePerItemChanging) && quantitySelector) {
        ...
        const morphTargets = [
          ...
          ['hz-price-per-item', pricePerItem, quantitySelectorWrapper],
        ];
